...
<section class="py-4 bg-light border-bottom">
    <div class="container-fluid">
        <form action="{{ route('search') }}" method="GET" class="row g-2 g-md-3 align-items-end justify-content-center">
            <div class="col-12 col-sm-6 col-md-3">
                <div class="position-relative">
                    <input type="text" id="searchInput" name="q" class="form-control" placeholder="Enter keyword..."
            ...
            @foreach ($grouped as $type => $plants)
                <div class="mb-5">
                    <h3 class="mb-4 fw-bold border-bottom pb-2
                        {{ strtolower($type) == 'family' ? 'text-warning' :
                           (strtolower($type) == 'genus' ? 'text-primary' : 'text-success') }}">
                        {{ strtoupper($type) }}
                    </h3>
                                    ...
                                    <a href="#" class="text-decoration-none text-dark">
                                        ...
                                        @else
                                            <img src="{{ asset('storage/images/species.jpg') }}"
                                                 class="plant-img w-100" alt="No Image">
                                        @endif

                                        <div class="p-3">
                                            <div class="plant-type
                                                {{ strtolower($plant['type']) == 'family' ? 'type-family' :
                                                   (strtolower($plant['type']) == 'genus' ? 'type-genus' : 'type-species') }}">
                                                🌿 {{ $plant['type'] }}
                                            </div>
        ...
        fetch(`{{ route('search.suggest') }}?q=${query}`)
            ...
            .then(data => {
                ...
                Object.keys(data).forEach(type => {
                    ...
                    groupTitle.className = "list-group-item active text-uppercase fw-bold";
